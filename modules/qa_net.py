...
class Model(nn.Module):
    ...
    def __init__(self, param):
        ...
        self.c_enc = EncoderBlock(conv_num=4, d=self.hidden_size, k=7, length=Max_Content_len, dropout_p=self.dropout_p)
        self.q_enc = EncoderBlock(conv_num=4, d=self.hidden_size, k=7, length=Max_Question_len, dropout_p=self.dropout_p)
        ...
        self.model_enc_blks = [EncoderBlock(conv_num=2, d=self.hidden_size, k=5, length=Max_Content_len,
                                            dropout_p=self.dropout_p) for _ in range(7)]
...
class SelfAttention(nn.Module):
    def __init__(self, d, heads_num):
        ...
        self.Wqs = [torch.nn.Linear(d, self.dv) for _ in range(heads_num)]
        self.Wks = [torch.nn.Linear(d, self.dv) for _ in range(heads_num)]
        self.Wvs = [torch.nn.Linear(d, self.dv) for _ in range(heads_num)]
...
class EncoderBlock(nn.Module):
    def __init__(self, conv_num, d, k, length, dropout_p):
        ...
        self.convs =[DepthwiseSeparableConv(d, d, k) for _ in range(conv_num)]
        self.self_att = SelfAttention()
        self.fc = nn.Linear(d, d)
        self.pos = PosEncoder(length, d)
        self.norm_1 = nn.LayerNorm([d, length])
        self.norm_2 = [nn.LayerNorm([d, length]) for _ in range(conv_num)]
...
class Pointer(nn.Module):
    ...
    def forward(self, M0, M1, M2, mask):
        ...
        x1 = utils.mask_logits(x1, mask)
        x1 = f.softmax(x1, dim=1)

        x2 = self.w2(x2).squeeze()
        x2 = utils.mask_logits(x2, mask)
